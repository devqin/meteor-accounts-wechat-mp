...
var getTokenResponse = function(config, query) {
  ...
  try {
    ...
    throw _.extend(new Error("Failed to complete OAuth handshake with WeChatMP. " + err.message), { response: err.response });
  }
...
var getIdentity = function(accessToken, openId) {
  try {
    ...
    throw _.extend(new Error("Failed to fetch identity from WeChatMP. " + err.message), { response: err.response });
  }
};
